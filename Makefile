...
build: restore
	dotnet build ./jaytwo.FluentHttp.sln
...
unit-test: build
	...
	rm -rf out/coverage
	cd ./test/jaytwo.FluentHttp.Tests; \
		dotnet test \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.FluentHttp.Tests.trx"
	reportgenerator \
